...
export default async (req, context) => {
  ...
  try {
    ...
    const { reviewText, tone = 'professional', gender = 'male', businessProfile = {} } = body

    if (!reviewText || reviewText.trim().length < 5) {
      return Response.json({ error: 'נא להזין טקסט ביקורת' }, { status: 400, headers })
    }

    const apiKey = process.env.OPENAI_API_KEY
    if (!apiKey) {
      return Response.json({ error: 'OpenAI API key not configured' }, { status: 500, headers })
    }
...
- החזר רק את טקסט התגובה, ללא הסברים נוספים.`
    ...
    if (!res.ok) {
      ...
      return Response.json({ error: 'שגיאה בשירות ה-AI'}, { status: 502, headers })
    }
    ...
    if (!response) {
      return Response.json({ error: 'לא התקבלה תגובה מה-AI' }, { status: 502, headers })
    }

    return Response.json({ response }, { headers })
  } catch (err) {
    return Response.json({ error: 'שגיאה פנימית', detail: err.message, stack: err.stack }, { status: 500, headers })
  }
}
